...
void SPjcTabMain::Construct(
	...
	const TSharedRef<FWorkspaceItem> AppMenuGroup
		= TabManager->AddLocalWorkspaceMenuCategory(FText::FromString(PjcConstants::ModulePjcName.ToString()));
	...
	MenuBarBuilder.AddPullDownMenu(
		FText::FromString(TEXT("Tabs")), FText::GetEmpty(), FNewMenuDelegate::CreateRaw(this, &SPjcTabMain::CreateMenuBarTabs, TabManager), "Window"
	);
	MenuBarBuilder.AddPullDownMenu(
		FText::FromString(TEXT("Help")), FText::GetEmpty(), FNewMenuDelegate::CreateRaw(this, &SPjcTabMain::CreateMenuBarHelp, TabManager), "Tabs"
	);

	// clang-format off
	ChildSlot
	[
		SNew(SWidgetSwitcher).WidgetIndex_Raw(this, &SPjcTabMain::GetWidgetIndex)
...
int32 SPjcTabMain::GetWidgetIndex() const {
	return UPjcSubsystem::EditorIsInPlayMode() || UPjcSubsystem::GetModuleAssetRegistry().Get().IsLoadingAssets() ? PjcConstants::WidgetIndexWorking
																												  : PjcConstants::WidgetIndexIdle;
}
