	...
	if (InColumnName.IsEqual(TEXT("AssetName"))) {
		...
		return SNew(SHorizontalBox)
			+ SHorizontalBox::Slot()
				  ...
				  .Padding(5.0, 0.0f)
				  .HAlign(HAlign_Left)
				  .VAlign(VAlign_Center)[SNew(SBox).WidthOverride(16).HeightOverride(16)[AssetThumbnail->MakeThumbnailWidget(ThumbnailConfig)]]
			+ SHorizontalBox::Slot()
				  .FillWidth(1.0f)
				  .Padding(5.0f, 0.0f)
				  .HAlign(HAlign_Left)
	...
	if (InColumnName.IsEqual(TEXT("FilePath"))) {
		return SNew(SHorizontalBox)
			+ SHorizontalBox::Slot()
				  ...
				  .Padding(FMargin {5.0f, 0.0f})[SNew(SHyperlink).Text(FText::FromString(Item->FilePath)).OnNavigate_Lambda([&]() {
