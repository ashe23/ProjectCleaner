...
TSharedRef<SWidget> SPjcItemStat::GenerateWidgetForColumn(const FName& InColumnName) {
	if (InColumnName.IsEqual(TEXT("Name"))) {
		return SNew(SHorizontalBox)
			+ SHorizontalBox::Slot().AutoWidth().Padding(Item->NamePadding)[SNew(STextBlock)
	...
	if (InColumnName.IsEqual(TEXT("Num"))) {
		return SNew(STextBlock).Justification(ETextJustify::Center).Text(Item->Num).ColorAndOpacity(Item->TextColor).ToolTipText(Item->ToolTipNum);
	}

	if (InColumnName.IsEqual(TEXT("Size"))) {
		return SNew(STextBlock).Justification(ETextJustify::Center).Text(Item->Size).ColorAndOpacity(Item->TextColor).ToolTipText(Item->ToolTipSize);
	}
