...
void SPjcTabAssetsIndirect::Construct(const FArguments& InArgs) {
	...
	Cmds->MapAction(FPjcCmds::Get().Refresh, FExecuteAction::CreateRaw(this, &SPjcTabAssetsIndirect::OnRefresh));
	...
	[
		...
		[
			...
			+ SVerticalBox::Slot()
			.AutoHeight()
			.Padding(FMargin{10.0f, 0.0f, 5.0f, 5.0f})
			[
				SNew(STextBlock)
		...
		[
			...
			.Padding(5.0f)
			[
				SNew(SWidgetSwitcher).WidgetIndex_Raw(this, &SPjcTabAssetsIndirect::GetWidgetIndex)
...
TSharedRef<SHeaderRow> SPjcTabAssetsIndirect::GetHeaderRow() {
	return SNew(SHeaderRow)
		+ SHeaderRow::Column(TEXT("AssetName"))
			  ...
			  .OnSort_Raw(
				  this, &SPjcTabAssetsIndirect::OnListSort
			  )[SNew(STextBlock)
...
SPjcTabAssetsIndirect::OnGenerateRow(TSharedPtr<FPjcAssetIndirectInfo> Item, const TSharedRef<STableViewBase>& OwnerTable) const {
...
void SPjcTabAssetsIndirect::ListUpdateData() {
	...
	for (const auto& AssetIndirectInfo : AssetIndirectInfos) {
		const TSharedPtr<FPjcAssetIndirectInfo> Item
			= MakeShareable(new FPjcAssetIndirectInfo {AssetIndirectInfo.Asset, AssetIndirectInfo.FilePath, AssetIndirectInfo.FileNum});
		if (Item.IsValid()) {
...
void SPjcTabAssetsIndirect::OnListSort(EColumnSortPriority::Type SortPriority, const FName& ColumnName, EColumnSortMode::Type InSortMode) {
	...
	if (ColumnName.IsEqual(TEXT("AssetName"))) {
		SortListItems(ColumnSortModeAssetName, [&](const TSharedPtr<FPjcAssetIndirectInfo>& Item1, const TSharedPtr<FPjcAssetIndirectInfo>& Item2) {
			return ColumnSortModeAssetName == EColumnSortMode::Ascending ? Item1->Asset.AssetName.ToString() < Item2->Asset.AssetName.ToString()
	...
	if (ColumnName.IsEqual(TEXT("AssetPath"))) {
		SortListItems(ColumnSortModeAssetPath, [&](const TSharedPtr<FPjcAssetIndirectInfo>& Item1, const TSharedPtr<FPjcAssetIndirectInfo>& Item2) {
			return ColumnSortModeAssetPath == EColumnSortMode::Ascending ? Item1->Asset.PackagePath.ToString() < Item2->Asset.PackagePath.ToString()
