...
public:
	...
	FContentFolderVisitor(
		...
		: FDirectoryVisitor(EDirectoryVisitorFlags::None),
		  ...
		  FoldersIgnored(InFoldersIgnored)
	{
	}
...
private:
	...
	bool CanScanFolder(const FString& InPath) const
	{
		...
		// ignored folders cant be scanned also
		ForbiddenFolders.Append(FoldersIgnored);
	...
	{
		TSet<FString> ForbiddenFolders;
		ForbiddenFolders.Add(FPaths::GameDevelopersDir());
		ForbiddenFolders.Add(FPaths::GameUserDeveloperDir());

		// ignored folders cant be deleted also
		ForbiddenFolders.Append(FoldersIgnored);
...
void FProjectCleanerScanner::Scan(const TWeakObjectPtr<UProjectCleanerScanSettings>& InScanSettings)
{
	...
	{
		FoldersIgnored.Add(FPaths::ProjectContentDir() / TEXT("Developers"));
	}

	const FString PathProjectContent = FPaths::ProjectContentDir();
	IPlatformFile& PlatformFile = FPlatformFileManager::Get().GetPlatformFile();
	FContentFolderVisitor ContentFolderVisitor(PlatformFile, FilesAll, FilesCorrupted, FilesNonEngine, FoldersAll, FoldersEmpty, FoldersIgnored);
...
void FProjectCleanerScanner::FindAssetsWithExternalRefs()
{
	...
	for (const auto& Asset : AssetsAll)
	{
		...
		{
			return !Ref.ToString().StartsWith(ProjectCleanerConstants::PathRelativeRoot);
		});
