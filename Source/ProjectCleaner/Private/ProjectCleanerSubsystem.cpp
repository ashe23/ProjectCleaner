...
bool UProjectCleanerSubsystem::IsAssetUnused(const FAssetData& AssetData) const
{
	if (!AssetData.IsValid()) return false;

	// if primary => false
	// if dependency of primary => false
	// if indirect => false
	// if dependency of indirect => false
	// if forbidden => false
	// if dependency of forbidden => false
	// if excluded => false
	// has external refs => false
	// under MSPresets folder => false
...
FProjectCleanerScanData UProjectCleanerSubsystem::ProjectScan(const FProjectCleanerScanSettings& ScanSettings)
{
	...
	SlowTaskScan.EnterProgressFrame(1.0f, FText::FromString(TEXT("Searching primary assets...")));
	
	TArray<FAssetData> AssetsPrimary;
	GetAssetsPrimary(AssetsPrimary);

	SlowTaskScan.EnterProgressFrame(1.0f, FText::FromString(TEXT("Searching indirect assets...")));
	
	// TArray<FProjectCleanerIndirectAsset> AssetsIndirect;
	// GetAssetsIndirect(AssetsIndirect);

	// getting all assets from AssetRegistry
	// TArray<FAssetData> AssetsAll;
	// ModuleAssetRegistry->Get().GetAssetsByPath(ProjectCleanerConstants::PathRelRoot, AssetsAll);


	// TArray<FAssetData> AssetsAllFiltered;
	//
	// FARFilter FilterAll;
	// FilterAll.bRecursivePaths = true;
	// FilterAll.PackagePaths.Add(ProjectCleanerConstants::PathRelRoot);
	// ModuleAssetRegistry->Get().GetAssets(FilterAll, AssetsAll);
	//
	// // excluding assets and paths that are forbidden to scan
	// FARFilter FilterExclude;
	// FilterExclude.bRecursivePaths = true;
	//
	// for (const auto& ForbiddenFolder : FoldersForbidden)
	// {
	// 	FilterExclude.PackagePaths.AddUnique(ForbiddenFolder);
	// }
	//
	// for (const auto& ForbiddenAsset : AssetsForbidden)
	// {
	// 	FilterExclude.ObjectPaths.AddUnique(ForbiddenAsset.ObjectPath);
	// }
	//
	// ModuleAssetRegistry->Get().UseFilterToExcludeAssets(AssetsAll, FilterExclude);

	// SlowTaskScan.EnterProgressFrame(1.0f, FText::FromString(TEXT("")));
...
void UProjectCleanerSubsystem::GetAssetsPrimary(TArray<FAssetData>& Assets)
{
	Assets.Reset();
	...
	for (const auto& BlueprintAsset : BlueprintAssets)
	{
		...
		{
			Assets.AddUnique(BlueprintAsset);
		}
	}
}
...
// 	if (FModuleManager::Get().IsModuleLoaded(TEXT("MegascansPlugin")))
// 	{
// 		FoldersForbidden.Add(TEXT("/Game/MSPresets"));
// 	}
// 	// todo:ashe23 for ue5 add __ExternalObject__ and __ExternalActors__ folders
// 	FoldersForbidden.Add(TEXT("/Game/Collections"));
//
