...
void SProjectCleanerTabNonEngine::Construct(const FArguments& InArgs)
{
	...
	Cmds->MapAction(
		...
		FUIAction(
			FExecuteAction::CreateLambda([&]()
			{
				...
				{
					UProjectCleanerLibNotification::Show(ProjectCleanerConstants::MsgFileDeleteSuccess, EProjectCleanerModalStatus::OK, 3.0f);
				}
				else
				{
					UProjectCleanerLibNotification::ShowOutputLog(ProjectCleanerConstants::MsgFileDeleteError, EProjectCleanerModalStatus::Error, 4.0f);
				}
	...
	GEditor->GetEditorSubsystem<UProjectCleanerSubsystem>()->OnScanFinished().AddLambda([&]()
	{
		ListUpdate();
	});
...
void SProjectCleanerTabNonEngine::ListUpdate()
{
	...
	for (const auto& NonEngineFile : GEditor->GetEditorSubsystem<UProjectCleanerSubsystem>()->GetFilesNonEngine())
	{
